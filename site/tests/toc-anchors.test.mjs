...
test("every table-of-contents entry targets a heading that exists", async () => {
  // The sidebar lists the whole curriculum, so the corpus is discovered rather
  // than hard-coded and newly added chapters are covered automatically.
  const slugs = documentSlugs(await htmlFor("/docs/curriculum"));
  assert.ok(slugs.length > 40, `expected the full curriculum, found ${slugs.length}`);
