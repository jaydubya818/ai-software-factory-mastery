...
are not redirected by that compatibility rule. The architecture alias is the
exception: the migration bridge may normalize it while host isolation is active,
and an explicitly standalone local runtime may redirect it to `/architecture`,
but a deployed Guide Preview retains `/guide/architecture` because FDLC owns the
bare route under Microfrontends. A copied alias that is never followed therefore
does not gain old-Guide compatibility. The internal `/guide/...` tree still
serves FDLC composition. Transitional root
