...
const views: ArchitectureView[] = [
  ...
  },
  {
    ...
    items: [
      ...
      item(["02", "Control plane", "Owns domain state, admission, policy, dispatch eligibility, and reconciliation.", "Control owner", "Commands and state transitions", "Critical", "Policy decision, expected version, audit event"], "/docs/03-build/13-control-plane-orchestrator-and-execution-plane", ["authority", "runtime"]),
      item(["03", "Execution plane", "Runs frozen manifests in isolated environments and reports structured results.", "Runtime owner", "Attempt contract", "Critical", "Tool calls, artifacts, side effects, completion"], "/docs/03-build/17-development-environments-sandboxes-and-compute", ["runtime"]),
      item(["04", "Knowledge plane", "Registers sources, retrieves eligible facts, and freezes attributable context.", "Knowledge owner", "Context package", "High", "Source, permission, selection, freshness, lineage"], "/docs/03-build/19-data-knowledge-and-semantic-engineering", ["runtime", "assurance"]),
      item(["05", "Quality plane", "Independently evaluates exact candidates and converts eligible results into proof.", "Quality owner", "Quality contract", "Critical", "Evaluator independence, provenance, contradiction"], "/docs/04-prove/27-quality-and-evidence-architecture", ["assurance"]),
      item(["06", "Delivery and outcome", "Binds approvals to artifacts, controls rollout, and observes production reality.", "Release and outcome owners", "Release and outcome contracts", "Critical", "Artifact, rollout, rollback, observation window"], "/docs/04-prove/32-cicd-progressive-delivery-and-production-verification", ["operations", "assurance"]),
    ],
  },
  {
    id: "components",
    ...
    items: [
      item(["01", "Routing and workflow", "Choose an eligible path and advance its durable graph.", "Runtime owner", "Route plus state command", "High", "Decision reason and transition event"], "/docs/03-build/13-control-plane-orchestrator-and-execution-plane", ["runtime"]),
      item(["02", "Context and retrieval", "Compile instructions and permission-filtered knowledge for one attempt.", "Knowledge owner", "Context package", "High", "Selected/excluded sources and package digest"], "/docs/03-build/20-context-engineering", ["runtime", "assurance"]),
      item(["03", "Models and tools", "Route qualified inference and authorize narrow side effects.", "AI and capability owners", "Model profile and tool contract", "Critical", "Exact versions, grants, calls, receipts, cost"], "/docs/03-build/11-the-agent-factory", ["runtime", "authority"]),
      item(["04", "State and memory", "Preserve working state without promoting observations into authority.", "Runtime owner", "Versioned state snapshot", "High", "State version, retention, provenance"], "/docs/03-build/18-agent-architecture", ["runtime"]),
      ...
      item(["06", "Reliability, evidence, and budgets", "Recover safely, stop nonconvergence, and attribute complete cost.", "Platform operations", "Recovery and budget contract", "Critical", "Stop reason, reconciliation, proof, charge ledger"], "/docs/05-operate/36-resilience-incidents-and-the-control-tower", ["operations", "assurance"]),
    ],
  },
  {
    id: "governance",
    ...
    items: [
      ...
      item(["02", "Risk and autonomy", "Classify consequence and enforce the narrowest current ceiling.", "System and policy owners", "Risk decision", "Critical", "Tier, permitted/prohibited actions, approval rule"], "/docs/03-build/23-agent-and-loop-engineering", ["authority"]),
      ...
      item(["04", "Attack resistance", "Contain indirect instructions, tool abuse, poisoned context or memory, and supply-chain change.", "Security owner", "Threat-control mapping", "Critical", "Negative tests, denials, quarantine, impact analysis"], "/docs/04-prove/33-security", ["authority", "assurance"]),
  ...
  },
  {
    ...
    items: [
      ...
      item(["03", "Data and trust", "Sources, classification, residency, retention, providers, credentials, boundaries, and downstream effects.", "Data and security owners", "Data and integration references", "Critical", "Access, lineage, residency, retention evidence"], "/docs/04-prove/33-security", ["authority", "assurance"]),
  ...
  },
  {
    ...
    items: [
      item(["00", "Deterministic", "Fixed rules and automation for stable, testable work.", "Software owner", "Code and tests", "Variable", "Deterministic test suite and change control"], "/docs/03-build/23-agent-and-loop-engineering", ["runtime", "assurance"]),
      item(["01", "Assisted", "Prompted drafting or explanation without direct side effects.", "Product owner", "Advisory response", "Low", "Human acceptance for consequence"], "/docs/03-build/23-agent-and-loop-engineering", ["assurance"]),
      item(["02", "Retrieval grounded", "Current approved sources and citations are required.", "Knowledge owner", "Context package", "High", "Permission, freshness, citation, contradiction"], "/docs/03-build/20-context-engineering", ["runtime", "assurance"]),
      item(["03", "Bounded single agent", "Iterative planning and tool use inside hard scope and budgets.", "Workflow owner", "Attempt manifest", "High", "Independent validators and stop conditions"], "/docs/03-build/23-agent-and-loop-engineering", ["runtime", "assurance"]),
      item(["04", "Coordinated specialists", "Specialization, parallelism, or independent challenge has measured value.", "Workflow owner", "Delegation graph", "High", "Handoff, disagreement, independence, parent budget"], "/docs/03-build/23-agent-and-loop-engineering", ["runtime", "assurance"]),
      item(["05", "Durable autonomous workflow", "Triggered long-running work must survive queues, crashes, and human pauses.", "Platform operations", "Durable workflow graph", "Critical", "Leases, reconciliation, gates, SLOs, emergency control"], "/docs/03-build/13-control-plane-orchestrator-and-execution-plane", ["runtime", "operations"]),
      item(["06", "Enterprise integrated", "Mission-critical work crosses governed data, tools, delivery, and operations.", "System owner", "Factory system contract", "Critical", "Inventory, policy, continuity, monitoring, recertification"], "/docs/05-operate/36-resilience-incidents-and-the-control-tower", ["authority", "operations", "assurance"]),
    ],
  },
  {
    id: "monitoring",
    ...
    items: [
      item(["01", "Observe and evaluate", "Correlate traces, logs, metrics, cost, quality, policy, evidence, incidents, and outcomes.", "Reliability and quality owners", "Signal and evaluation records", "High", "Coverage, baseline, uncertainty, subject binding"], "/docs/05-operate/36-resilience-incidents-and-the-control-tower", ["operations", "assurance"]),
      item(["02", "Detect and triage", "Distinguish normal variation, drift, control failure, and incident.", "On-call owner", "Finding", "Critical", "Rule version, severity, owner, deadline, evidence"], "/docs/05-operate/36-resilience-incidents-and-the-control-tower", ["operations"]),
  ...
  },
  {
    ...
    items: [
      item(["01", "Register and ingest", "Approve source ownership, connector identity, classification, freshness, and correction policy.", "Knowledge and source owners", "SourceRegistration", "High", "Checkpoint, source version, transformation lineage"], "/docs/03-build/19-data-knowledge-and-semantic-engineering", ["authority", "runtime"]),
      item(["02", "Filter and retrieve", "Apply tenant, purpose, permission, lifecycle, and freshness before ranking.", "Security and knowledge owners", "RetrievalRequest", "Critical", "Permission decisions, candidates, exclusions"], "/docs/03-build/20-context-engineering", ["runtime", "assurance"]),
      item(["03", "Rank and compile", "Rerank, diversify, preserve contradiction, allocate tokens, and freeze the package.", "Context owner", "ContextSelection", "High", "Why selected, citations, digest, unresolved facts"], "/docs/03-build/20-context-engineering", ["runtime", "assurance"]),
      item(["04", "Evaluate and revoke", "Connect context to outcomes and propagate correction, deletion, or compromise.", "Knowledge and quality owners", "Revocation event", "Critical", "Reverse lineage, affected work, deletion and rebuild receipts"], "/docs/03-build/20-context-engineering", ["operations", "assurance"]),
    ],
  },
  {
    id: "evidence",
    ...
    items: [
      ...
      item(["02", "Execution", "Attempt, context, tool calls, side effects, and artifacts establish what happened.", "Runtime owner", "Attempt and capability records", "High", "Exact versions, environment, receipts, digests"], "/docs/03-build/13-control-plane-orchestrator-and-execution-plane", ["runtime"]),
      item(["03", "Assurance", "Independent evaluators and proof packages establish which claims are eligible.", "Quality owner", "Proof package", "Critical", "Subject binding, provenance, independence, freshness"], "/docs/04-prove/31-quality-contracts-proof-packages-and-certificates", ["assurance"]),
