...
export function TopicExplorer({ documents }: { documents: readonly TopicDocument[] }) {
  ...
  const [view, setView] = useState<"list" | "map">("list"); const [restored, setRestored] = useState(false);
  ...
  function selectSystemNode(node: (typeof systemNodes)[number]) { reset(); if (node.key === "section") setSection(node.value); if (node.key === "lifecycle") setLifecycle(node.value); if (node.key === "architecture") setArchitecture(node.value); if (node.key === "query") setQuery(node.value); }
