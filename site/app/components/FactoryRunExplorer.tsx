...
function KnowledgeCheck() {
  ...
  return (
    <section className="run-knowledge-check" aria-labelledby="run-check-title">
      ...
      {answer && <p className={correct ? "is-correct" : "is-incorrect"} role="status"><strong>{correct ? "Correct." : "Not quite."}</strong> Completion comes from execution. Acceptance depends on independent evidence and the authority named by policy. <Link href="/docs/04-prove/21-quality-and-evidence-architecture">Review the boundary →</Link></p>}
    </section>
  );
}
