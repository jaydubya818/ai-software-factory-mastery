...
import { guideContentPath, guideNavigationHref } from "../../lib/paths";
...
export function DocumentNav({ currentSlug, sections }: { currentSlug: string; sections: NavSection[] }) {
  ...
  const content = <div className="document-nav-groups">{sections.map((section) => <section key={section.key}><h2>{section.label}</h2><ul>{section.documents.map((document) => <li key={document.slug}><a aria-current={document.slug === currentSlug ? "page" : undefined} href={guideNavigationHref(guideContentPath(document.slug))}>{document.title}</a></li>)}</ul></section>)}</div>;
