  ...
  return (
    <>
      ...
      <main className="interior-page topics-page">
          ...
          {shelf.map((row) => (
            <li className="shelf-row" key={row.letter} id={row.slug.split("/")[1]}>
              ...
              <div className="shelf-body">
                <Link className="shelf-title" href={guideContentPath(row.slug)}><h2>{row.title}</h2><span aria-hidden="true">→</span></Link>
                <p>{row.description}</p>
                {row.children && (
                  <ul className="shelf-children">
                    {row.children.map((child) => <li key={child.slug}><Link href={guideContentPath(child.slug)}><strong>{child.title}</strong><span>{child.note}</span></Link></li>)}
                  </ul>
                )}
              </div>
        ...
        <p className="reference-count">{appendices.length} reference documents. Chapters live in the <Link href={GUIDE_ROUTES.home}>table of contents</Link>.</p>
      </main>
      <SiteFooter />
    </>
  );
