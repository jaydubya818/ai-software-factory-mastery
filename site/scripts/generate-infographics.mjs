...
function metadata(record) {
  if (record.references.length > 1) {
    return record.references.map(({ chapter }) => chapter ? `CHAPTER ${String(chapter).padStart(2, "0")}` : "STAGE").join(" + ") + " · CROSS-CUTTING";
  }
  const label = record.relativeFile.startsWith("stages/") ? `STAGE ${record.chapter}` : `CHAPTER ${String(record.chapter).padStart(2, "0")}`;
...
console.log(requestedSlots.size ? `Done: ${records.length} selected PNGs.` : `Done: ${records.length} PNGs cover 187 chapter references.`);
