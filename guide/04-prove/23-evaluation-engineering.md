...
An eval is a factory asset like a skill or a verifier, and the registry is its entry in the factory asset lifecycle of [Chapter 10](../03-build/10-the-agent-factory.md): owner → version → evaluate → deploy → observe → improve → deprecate. The "evaluate" step for an eval is validating it against production; "deprecate" is what the expiry date forces someone to decide.
...
- Where the model × harness matrix is defined: [13. Coding harnesses and agent protocols](../03-build/13-coding-harnesses-and-agent-protocols.md). The asset lifecycle the eval registry belongs to: [10. The agent factory](../03-build/10-the-agent-factory.md).
