...
A **hermetic build** is one whose inputs are fully declared and fetched from pinned, content-addressed sources, so the same inputs produce the same bytes anywhere. Pinned dependencies, isolated ephemeral builders, and controlled inputs make the artifact reproducible; [Chapter 26](./26-security.md) covers the provenance and attestation that make it verifiable. Hermeticity is expensive for legacy stacks, so apply it in proportion to consequence: the artifact that handles payments earns a hermetic build before the internal admin tool does. What is not negotiable, even for the admin tool, is digest identity. Tags locate an artifact; digests identify it.
...
**Related chapters.** [24. Quality contracts, proof packages, and certificates](./24-quality-contracts-proof-packages-and-certificates.md) defines the eligibility this chapter consumes. [26. Security](./26-security.md) covers builder hardening, provenance, and attestation. [28. Observability, telemetry, and forensics](../05-operate/28-observability-telemetry-and-forensics.md) supplies the signals. [29. Resilience, incidents, and the control tower](../05-operate/29-resilience-incidents-and-the-control-tower.md) takes over when a stop condition becomes an incident. [32. Production feedback, review, and the agentic merge queue](../06-improve/32-production-feedback-review-and-the-agentic-merge-queue.md) covers the merge side. [12. Durable execution](../03-build/12-durable-execution.md) explains the leases and events the factory SLOs measure. [9. Multi-repository design](../02-design/09-multi-repository-design.md) covers cross-repository release ordering. [3. First principles](../01-understand/03-first-principles-trust-evidence-and-authority.md) for the autonomy ladder that error budgets move. Terms are in the [glossary](../appendix/glossary.md).

**Labs.** [Lab 6 — Progressive delivery and rollback](../appendix/labs/06-progressive-delivery-and-rollback-lab.md) builds an immutable artifact, exposes it to a canary, triggers a failure, and executes rollback or containment. [Lab 7 — Incident remediation and postmortem](../appendix/labs/07-incident-remediation-and-postmortem-lab.md) continues from a delayed production failure.
