...
**The capability supply chain, as tooling now practises it.** Skills as versioned packages with a manifest, lockfile, registry, workspaces, and typed input/output schemas; the skill inventory; install policy (security threshold, source allowlist, minimum release age); reviewer rubrics and a scored quality gate; with-and-without context evals; and the mining of review history into rules, skills, verifiers, and refactors are in [Chapter 10](../03-build/10-the-agent-factory.md), [Chapter 23](../04-prove/23-evaluation-engineering.md), [Chapter 26](../04-prove/26-security.md), [Chapter 32](../06-improve/32-production-feedback-review-and-the-agentic-merge-queue.md), and [Chapter 33](../06-improve/33-governed-learning-and-compounding-engineering.md), with public tooling documentation cited as the example.

**Mission Control at its current revision.** The seven operating principles, the eleven-stage gate table, the eight-actor ownership table, the inequality chain from execution to production verification, the proof record, and the current limitations are in [Chapter 34](../06-improve/34-mission-control-as-a-living-case-study.md), pinned to the commit named there.
