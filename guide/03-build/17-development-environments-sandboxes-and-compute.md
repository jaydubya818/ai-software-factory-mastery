...
# 17. Development environments, sandboxes, and compute
...
The Execution Manifest from [Chapter 13](./13-control-plane-orchestrator-and-execution-plane.md) binds an exact environment version, by image digest and toolchain digest, to each Attempt. That second digest is the product of **toolchain pinning**: every compiler, runtime, package manager, linter, and browser the run can invoke is fixed to an exact version and recorded, so a build that passed on Tuesday cannot silently start failing on Thursday because a tool auto-updated underneath it. A floating "latest" image or mutable workstation state makes reproduction meaningless, because you can never again stand in the room the agent stood in.
...
External taxonomies sometimes absorb the environment into an “outer harness.” FDLC keeps the boundaries separate. The **Agent Harness** governs how the Agent operates. The **Runtime** hosts execution and manages lifecycle, leases, checkpoints, resume, credentials delivery, filesystems, and networks. The **development environment** is the provisioned toolchain, checkout, services, and preview surface. The **Sandbox** is the isolation boundary that limits effects. The Control Plane owns the authority behind the workload identity and scoped grants; qualified infrastructure enforces them. This separation makes a failure diagnosable and each component independently versionable.
...
| **Tool permissions** | Which tools and MCP servers the run may invoke, with which arguments and side-effect classes | Tool gateway and policy engine outside the model ([Chapter 18](./18-agent-architecture.md)) | A model turning a read tool into a write tool, or reaching a tool it was never granted |
...
| **Replay protection** | That a command, event, or credential cannot be replayed to produce a second effect | Idempotency keys, nonces, expiry on every control-to-execution message ([Chapter 14](./14-durable-execution.md)) | Duplicate publication; a captured token reused after the run ended |
...
Backoff and retry never authorize another external effect. Idempotency and reconciliation, from [Chapter 14](./14-durable-execution.md), remain required whenever allocation, publication, or teardown may have succeeded before the response was lost.
...
| **Data** | What context a run may see | Authorization applied before any content reaches the model, not after retrieval; a tenant's documents never enter another tenant's context window ([Chapter 19](./19-data-knowledge-and-semantic-engineering.md)) |
...
- [Chapter 13. Control plane, orchestrator, and execution plane](./13-control-plane-orchestrator-and-execution-plane.md) — the Execution Manifest that binds an environment to an Attempt.
- [Chapter 14. Durable execution](./14-durable-execution.md) — leases, idempotency, and recovery that fleet controls depend on.
- [Chapter 15. Coding harnesses and agent protocols](./15-coding-harnesses-and-agent-protocols.md) — the layer above.
- [Chapter 33. Security](../04-prove/33-security.md) — identity, secrets, threats, and supply chain in depth.
- [Chapter 35. Observability, telemetry, and forensics](../05-operate/35-observability-telemetry-and-forensics.md) — the operator view of provisioning, cost, and orphans.
- [Chapter 38. Enterprise adoption and the infrastructure landscape](../05-operate/38-enterprise-adoption-and-the-infrastructure-landscape.md) — providers and the enterprise checklist.
- [Chapter 7. Governance, policy, and risk-proportional approval](../02-design/07-governance-policy-and-risk-proportional-approval.md) and [Chapter 18. Agent architecture](./18-agent-architecture.md) — the policy layers and tool gateway that enforce the isolation model's rows from outside the model.
