...
- Ground first. An ungrounded agent fails slowly rather than cheaply: it searches one more place, re-bills its context every turn, and ends confidently wrong. A context graph, governed like any other source, is a cost control and a quality control at once. Trusted context is 80 percent of the agent's success.
...
- Context engineering precedes automation: you cannot reliably automate what you have not adequately defined. The Definition of Correct is the artifact an agent is handed before work and a verifier after it.
- Context posture is managed like dependency posture: an inventory of every context artifact with seven detections (duplicate, stale, conflicting, missing, unowned, outdated, unsafe), utility measured with and without, and pruning for what shows no delta.
- Context routing (which context does this task receive) is a router separate from model routing. Shift standards left to the producer so review becomes defence in depth. A context firewall separates creator from verifier, planner from executor, executor from security reviewer, and customer data from the general worker.
