...
infographics: [knowledge-pipeline, retrieval-evaluation, semantic-layer, context-graph, context-hierarchy, context-lifecycle, context-cdl, context-posture, context-firewall, sufficiency-loop]
---
...
| Procedural | Skills and workflows | The Agent Factory's registries ([Chapter 10](./10-the-agent-factory.md)) |
...
- Context engineering precedes automation: you cannot reliably automate what you have not adequately defined. The Definition of Correct (requirements, standards, policies, architecture, constraints, acceptance criteria, verification rules) is the artifact an agent is handed before work and a verifier after it; specificity climbs from generic to domain, repository, component, explicit definition, focused verifier.
- Context as code means ten verbs: version, review, test, evaluate, own, distribute, deprecate, detect drift, measure effectiveness, roll back. The lifecycle is create → review → version → distribute → execute → observe → evaluate → improve → deprecate; the Context CDL is its enforced implementation.
- Context posture is managed like dependency posture: an inventory of every context artifact with seven detections (duplicate, stale, conflicting, missing, unowned, outdated, unsafe). Context drift is documentation diverging from reality; deduplication is discover → compare → consolidate → source of truth → distribute; utility is measured with and without; pruning removes what shows no delta.
- Compensatory context is deleted on every model upgrade; institutional context ("how we work here") is protected through every one. The company brain is a governed source the compiler selects from; more context is not better context. Represent knowledge for agents as structured canonical knowledge, not as the human document.
- Memory is structured external state in six kinds: working, episodic, semantic, procedural, preference, and durable factory state, and the last is not memory but the authoritative records.
- Context routing (which context does this task receive) is a router separate from model routing. Shift standards left to the producer so review becomes defence in depth. A context firewall separates creator from verifier, planner from executor, executor from security reviewer, and customer data from the general worker; the verifier gets goal, artifact, and verification contract, never the producer's reasoning.
