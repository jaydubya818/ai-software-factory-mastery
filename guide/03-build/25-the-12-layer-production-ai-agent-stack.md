...
# 25. The 12-layer production AI agent stack
...
### 2. Data Understanding — [Chapter 19](./19-data-knowledge-and-semantic-engineering.md)
...
### 3. Knowledge Engineering — [Chapter 19](./19-data-knowledge-and-semantic-engineering.md)
...
### 4. Model Engineering — [Chapter 21](./21-models-and-capability-selection.md)
...
### 5. Context Engineering — [Chapter 19](./19-data-knowledge-and-semantic-engineering.md)
...
### 6. Semantic Engineering — [Chapter 19](./19-data-knowledge-and-semantic-engineering.md)
...
### 7. Agent Engineering — [Chapter 18](./18-agent-architecture.md) and [Chapter 23](./23-agent-and-loop-engineering.md)
...
### 8. Loop Engineering — [Chapter 23](./23-agent-and-loop-engineering.md)
...
### 9. Evaluation Engineering — [Chapter 29](../04-prove/29-evaluation-engineering.md)
...
### 10. Harness Engineering — [Chapter 15](./15-coding-harnesses-and-agent-protocols.md)

*Decides* how an Agent is allowed to operate and how execution is captured, controlled, replayed, and compared. *Inputs:* the frozen Agent Definition, Harness Version, Execution Profile, qualified Capability Implementations, and environment reference. *Outputs:* a run record, checkpoints, event trace, tool receipts, artifacts, session transcripts, structured completion, and replayable evidence. *Prevents* missing controls, irreproducible behavior, and unverifiable completion. The Agent Loop runs inside the Harness; the Orchestrator advances the Work Graph outside it; the Runtime hosts and preserves execution; the Sandbox isolates effects; and the Control Plane owns enterprise authority. *Example:* a reviewer asks why an agent deleted a migration file; the run record shows the exact tool call, context digest, capability version, Harness Version, and model route, and the team replays the Attempt against a mocked filesystem. *Lives in* the Agent Harness, with durable records persisted through the Runtime and Control Plane.

### 11. Infrastructure Engineering — [Chapter 14](./14-durable-execution.md) and [Chapter 17](./17-development-environments-sandboxes-and-compute.md)
...
### 12. Continual Learning — [Chapter 40](../06-improve/40-governed-learning.md)
...
A second class of failure is structural: layers collapsing into each other. The most common collapse is knowledge preparation, context selection, Harness behavior, Runtime behavior, and workflow governance compressed into one oversized "agent infrastructure" layer. When that happens, nobody can say which component selected a stale document, routed the wrong node, failed to resume, or granted an action, and the fix lands in the wrong place.

Keep the boundaries explicit. An **Agent** is a goal-directed worker. An **Agent Loop** governs its next-action cycle and stopping behavior. An **Agent Harness** governs how it interacts with models, context, tools, state, permissions, budgets, and external systems. A **Coding Harness** specializes that operating envelope for repository work; an **IDE Harness** is a Coding Harness integrated into an editor. A **Work Graph** represents workflow topology, and the **Orchestrator** advances it. A **Runtime** hosts and preserves execution. A **Sandbox** isolates effects. The **Control Plane** owns identity, authority, policy, approvals, budgets, revocation, promotion, and release. The **Agent Factory** versions and qualifies reusable agent assets and Capability Implementations. A **Software Delivery Factory** composes all of these to produce accepted outcomes. See [Execution boundaries and canonical terminology](../appendix/execution-boundaries-and-terminology.md).
...
- Layer chapters: [6. Intent and specification engineering](../02-design/06-intent-and-specification-engineering.md) · [19. Data, knowledge, semantic, and context engineering](./19-data-knowledge-and-semantic-engineering.md) · [21. Models](./21-models-and-capability-selection.md) · [18. Agent architecture](./18-agent-architecture.md) · [23. Agent and loop engineering](./23-agent-and-loop-engineering.md) · [29. Evaluation engineering](../04-prove/29-evaluation-engineering.md) · [15. Coding harnesses and agent protocols](./15-coding-harnesses-and-agent-protocols.md) · [14. Durable execution](./14-durable-execution.md) · [17. Development environments, sandboxes, and compute](./17-development-environments-sandboxes-and-compute.md) · [40. Governed learning and compounding engineering](../06-improve/40-governed-learning.md).
...
- [13. Control plane, orchestrator, and execution plane](./13-control-plane-orchestrator-and-execution-plane.md) for the runtime contracts the layers run on.
