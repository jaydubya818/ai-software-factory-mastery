...
- The loop stops when external evidence says stop: a goal condition, not a step count. The model never grades its own work.
- A loop decides whether execution continues; a graph decides where it goes next, through nodes, conditional edges, typed shared state, and a checkpoint after every node.
- Four defaults decide what a loop costs: a compaction threshold below the window, a medium reasoning effort, a cheaper default model for subagents, and a cache TTL matched to the gap between turns. Optimize the turns, requests, and tokens the agent spends on its own behalf; cost per outcome, never cost per token.
...
- A chain runs once; a loop acts, observes, reasons, and repeats until a testable termination. A well-engineered loop has five parts: a goal with a testable termination, a tool set that can reach it, context management across iterations, termination logic in both directions, and error handling that never repeats the identical failed action. A loop that retries the same action after the same error is spinning, not learning.
- Four patterns cover most loops: retry, plan–execute–verify, explore–narrow, human-in-the-loop. Define termination first, feed structured feedback that flags repeated errors, log everything and summarize often, treat an exhausted tool-call budget as a failure signal, and test the loop on unsolvable tasks to prove the exit works.
- Automation is Trigger → Execute → Output. A loop is Trigger → Context → Execute → Observe → Verify → Evaluate → Learn → Improve → next run. Loop engineering is the discipline of making workflows more reliable through repeated execution and measured feedback, from trigger design to controlled promotion.
