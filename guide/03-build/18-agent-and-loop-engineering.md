...
|---|---|---|
| **Compaction threshold** | Compact the session at 400K tokens, even on a 1M-context model | Past that point each turn re-sends an enormous prefix; a cache burst or a repeated-input bill costs more than the context it preserves. Chapter 13 covers what compaction does to evidence |
...
- Draw the work as a graph, not a line: nodes do the thinking, edges carry data and are code — agents propose, code disposes. Fan out where work is independent, barrier only where a stage needs the whole set, verify on the edge, tier models by where judgment lives. A loop decides whether execution continues; a graph decides where it goes next.
- Default to one agent; add a second only at a real boundary — different permission domains, context, or capabilities, meaningful parallelism, or deliberate producer/verifier independence. Agent count is an architectural cost, not a feature; never build a virtual org chart.
- Decomposition produces a task graph, not a pile of prompts: every task carries objective, inputs, outputs, dependencies, context and capability requirements, risk, verification, and retry semantics. Every handoff is a typed, versioned contract — conversational memory is not one, and a delegate cannot widen scope, re-delegate, or claim acceptance.
- The attempt loop is Generate → Verify → Diagnose → Repair or Replan → Retry → Escalate or Stop, and retry, repair, replan, fallback, escalation, and stop are different actions with different records. The loop stops when external evidence says stop — a goal condition, never a step count or the model's own opinion.
- A chain runs once; a loop acts, observes, reasons, and repeats until a testable termination. Its five required parts are a goal with a testable termination, a tool set that can reach it, context management across iterations, termination logic in both directions, and error handling that never repeats the identical failed action.
- Four runtime defaults decide what a loop costs: a compaction threshold well below the window, a medium reasoning effort, a cheaper default model for subagents, and a cache TTL matched to the gap between turns. Optimize the turns, requests, and tokens the agent spends on its own behalf — cost per outcome, never cost per token.
