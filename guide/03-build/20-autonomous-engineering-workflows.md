...
- One complete workflow exposing real weaknesses beats ten disconnected demos.
- Agent readiness is scored, not assumed: twelve dimensions (testability, CLI accessibility, build reproducibility, documentation quality, context quality, sandboxability, credential accessibility, observability, deterministic validation, architecture clarity, environment reproducibility, dependency health) assessed across eight families (context, tool, environment, test, observability, architecture, security, verification). The score predicts touchpoints and therefore affordable autonomy.
- The deterministic feedback surface is what an agent corrects against; its density is the readiness investment with the highest return. Architecture linting moves architecture from a document to an executable constraint. Factory-friendly architecture is ordinary good engineering with an unusual payback, and an agent affordance (CLI over UI, structured query over raw logs, environment manifest, structured errors) is the fix for a recurring touchpoint.
- The factory's boundary is signal-to-outcome, not ticket-to-code. Signals → Triage → Prioritisation → Intent → Plan → Execution → Verification → Deployment → Outcomes → Signals; work shaping and product taste stay human, and the loop exists to feed them well.
- An agent-legible workflow keeps every step as a retrievable artifact: ticket = intent, PR = proposed change, comments = feedback, CI = verification, merge = accepted outcome, incident = delayed outcome.
