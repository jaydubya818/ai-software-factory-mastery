---
title: "Routing and the escalation ladder"
part: build
chapter: 22
summary: "How policy routes each workload to the cheapest qualified capability, escalates on evidence, and preserves security and quality through fallback."
absorbs: [06-ai-engineering/02-model-routing-evaluations-and-capability-selection.md]
infographics: [model-router, escalation-ladder, adaptive-routing]
---

# 22. Routing and the escalation ladder

Routing is a policy decision over qualified model profiles, and one valid result is no model at all. This chapter defines the workload taxonomy, ordered eligibility filters, escalation ladder, fallback rules, operating lanes, and evidence required to change a route safely.

## The problem

No model is best for every operation. Stronger routes may cost more; cheaper routes may retry or shift work to reviewers; provider outages require fallback; switching can destroy cache value and builder intuition. Routing must optimize cost per trusted outcome without relaxing capability, security, or policy.
...
- Route only among qualified profiles; eligibility, capability, policy, and quality come before cost.
- A workload taxonomy maps task class and risk to a lane, quality floor, budget, fallback, and escalation threshold.
- Fallback may relax cost or latency, never capability, security, or policy; no eligible route is an explicit blocked state.
- Use deterministic automation first, then climb cheap → specialized → frontier only when the remaining judgment requires it.
- Every route decision and in-loop switch is recorded, evaluated by outcome, and promoted through shadow, canary, and human approval.

## Go deeper

- Related chapters: [18. Agent architecture](./18-agent-architecture.md) for the model literacy table and the execution manifest; [23. Agent and loop engineering](./23-agent-and-loop-engineering.md) for sub-agents on different models; [29. Evaluation engineering](../04-prove/29-evaluation-engineering.md); [40. Governed learning](../06-improve/40-governed-learning.md) for promotion gates; [36. Resilience](../05-operate/36-resilience-incidents-and-the-control-tower.md) for provider failover.
...
- Where the tier that sets the intelligence budget is defined: [7. Governance, policy, and risk-proportional approval](../02-design/07-governance-policy-and-risk-proportional-approval.md). Where the with/without test for parallel candidates lives: [29. Evaluation engineering](../04-prove/29-evaluation-engineering.md).
