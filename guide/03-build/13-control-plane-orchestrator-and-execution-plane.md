...
Strip away the formal vocabulary and experienced factory builders describe the same surface. The control plane's job list is concrete: dispatch new work; look at session traces; look at the plans and architecture docs being produced; schedule things to run every night, on a cron, or in response to webhooks; review and iterate on the code itself in a PR-shaped way that does not necessarily have to happen in GitHub; manage permissions and audit, who can talk to what and who can see which services; manage spend and budgeting; and, on top of all that, the compounding-engineering memory that turns "every engineer on the team is telling the agent the same thing" into a governed improvement proposal ([Chapter 40](../06-improve/40-governed-learning.md)).

One practical deployment shape is also instructive. Each developer machine runs a small web server exposing a REST API; a hosted web application listens to Linear, Slack, GitHub, and everything else, receives their webhooks, keeps the database, and talks to the per-machine servers. That hosted piece is the **dispatcher**, an orchestration component. It is the same shape as the diagram at the top of this chapter with the names changed.

Mission Control makes the same list from the governance side. The coding agent performs tasks; the control plane determines what work should happen, which agent should perform it, what tools it may use, what evidence is required, which human must approve, whether the outcome is safe, what it cost, what happened in production, and what the system should learn. That is the difference between "another coding-agent product" and the operating platform for autonomous engineering.
...
Some external sources call cross-Harness governance or adaptive workflow construction a “meta-harness.” FDLC does not use that as a canonical component. Cross-Harness authority belongs to the Control Plane; reusable assets belong to the Agent Factory; durable topology belongs to the Work Graph; coordination belongs to the Orchestrator; execution survival belongs to the Runtime; and isolation belongs to the Sandbox.

**Outcome-driven orchestration** constructs or selects the Work Graph needed for an outcome rather than executing a predetermined sequence. Given a goal, constraints, eligible Capability Implementations, and a verification contract, the Orchestrator may choose decomposition, workers, skills, strategy, and verification nodes within the frozen authority envelope. Its instruction has one shape: *produce X subject to Y and prove A through F before completion.* X is the outcome; Y is the constraints, budget, and scope; A through F are the claims in the verification contract ([Chapter 27](../04-prove/27-quality-and-evidence-architecture.md#the-verification-contract)).

Nothing in that changes authority. The Orchestrator chooses an eligible route; the Control Plane has already frozen what may be attempted, within which bounds, and what counts as proof. The Harness governs each selected Agent's operation. The Runtime hosts it. Independent Verification evaluates the resulting evidence. The more completely an outcome can be specified and independently verified, the more topology the Orchestrator may construct without a human prescribing each step.
...
- Source materials: public factory architecture discussions, internal architecture notes, the production-agent stack, and the Mission Control glossary and lexicon reviewed 2026-09-02.
