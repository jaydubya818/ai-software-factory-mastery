...
- Use smaller models for simpler work, and strong models only where reasoning creates value; that is model selection and routing ([Chapter 17](../03-build/17-models-routing-and-capability-selection.md)), including the escalation and downgrade decisions taken while a loop runs.
...
- Four factory dimensions: Autonomy (corrections per task, takeover rate, first-pass success) × Automation (agent-initiated share, touchpoints per accepted outcome, auto-merge share) × Quality (escaped defects, regression rate, acceptance rate) × Economics (cost, tokens, and human minutes per accepted outcome). Autonomy is not automation — a hundred correct PRs each approved by hand is a review bottleneck — and the target is the economically optimal level that meets the quality and risk threshold, not the maximum. The human leverage ratio is verified outcomes per unit of human attention.
- Tokenomics spans models, context, tools, workflows, retries, caching, parallelism, and verification: spend intelligence where intelligence creates value. Cost per verified outcome is the engineering sub-metric an engineer can move this week; cost per accepted outcome is the business metric known weeks later; the gap is what verification missed.
- Execution budgets come in seven kinds (tokens, spend, compute, time, tool calls, retries, model tier) at five nested levels (task, mission, repository, team, organisation), and reaching one stops, escalates, or requests authorisation. Predict cost before and during a run so the budget is sized and the ceiling is seen coming.
- Factory economics has ten cost lines — inference, compute, tooling, sandbox, storage, verification, retries, human review, failure remediation, opportunity cost — and six returns: velocity, quality, human leverage, backlog elimination, risk reduction, broader participation. Factory ROI is measured against the full cost of operating and improving the factory, on benefit lines that each point at a dashboard metric with a baseline.
- The bottleneck migrates: implementation → review → verification, context, intent, governance → factory engineering. Count latent quality capacity as a return, and role fungibility as accepted outcomes from people outside the role that used to own them.
