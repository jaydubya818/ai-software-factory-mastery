...
The two blocked September 4 attempts remain admission failures with zero trials. Subsequent inspection found that the isolated official 0.146.0 native binary matches the approved Darwin ARM64 digest. The adapter's npm wrapper resolver assumed a nested dependency directory, while this installation hoisted the platform package. Selecting the already approved native binary passed the unchanged version and digest checks. The original CLI containment remains unqualified. A separate minimal container/chroot candidate has now passed offline process-containment, environment and cleanup checks, alongside deterministic provider-adapter controls. This is a new runtime identity; its runtime and transmission envelope remain unadmitted. Approval for twelve synthetic OpenAI calls exists and remains unused. **No external model calls occurred, and behavioral effectiveness is not measured.** Technical admission remains the blocker; offline control results cannot establish real provider provenance or behavioral qualification. Monetary cost may remain UNKNOWN. External execution must stay disabled pending exact runtime, Execution Profile, model-route and transmission admission.
